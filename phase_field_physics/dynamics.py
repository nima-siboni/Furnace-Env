...
def calc_mobility(mobility_type: str, mu_0: float, Temperature: float, T_max: float, T_min: float):
    if mobility_type == 'constant':
        ...
        tmp = 0.5 * (T_max + T_min) / (T_max - 2. * T_min) + 1
        b = mu_0 / tmp
        a = b / (T_max - 2. * T_min)
...
def Calc_Force_AC(Phi: np.ndarray, Temperature: float, G_list: List, mobility_type: str, T_max: float = 1000.,
    ...
    :param mobility_type: the type of mobility which can be constant, linear, or exponential
    :param T_max: the maximum temperature (in C)
    :param T_min: the minimum temperature (in C)
    :param mu_0: a value for mobility which set the scale of the mobility (in different mobility types it is used
    differently.
